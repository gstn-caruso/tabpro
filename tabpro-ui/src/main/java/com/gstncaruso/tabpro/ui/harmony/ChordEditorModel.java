...
public final class ChordEditorModel {
    ...
    public ChordEditorModel(Tuning tuning) {
        this(tuning, ChordSelection.initial());
    }

    private ChordEditorModel(Tuning tuning, ChordSelection selection) {
        this.tuning = tuning;
    ...
    public static ChordEditorModel forBeat(Beat beat, Tuning tuning) {
        if (beat.effects().chord().isPresent()) {
            ChordEditorModel model = new ChordEditorModel(tuning);
        ...
        return new ChordEditorModel(tuning);
    }
    ...
    private void rebuildFromSelection() {
        candidates = ChordDiagramGenerator
                .generate(selection.chord(), tuning, ChordDiagramGenerator.DEFAULT_MAX_SPAN, selection.complexity())
                .stream()
                .filter(barrePreference::accepts)
                .toList();
        current = candidates.isEmpty() ? ChordDiagram.justTheName(selection.chord().name()) : candidates.get(0);
    ...
    public void pickAlternativeName(Chord chord) {
        ...
        custom = false;
        current = current.withName(chord.name());
        candidates = ChordDiagramGenerator
                .generate(selection.chord(), tuning, ChordDiagramGenerator.DEFAULT_MAX_SPAN, selection.complexity())
                .stream()
                .filter(barrePreference::accepts)
    ...
    private void loadDiagram(ChordDiagram diagram) {
        ...
        if (names.isEmpty()) {
            ...
            custom = false;
            current = current.withName(diagram.name().isBlank() ? best.name() : diagram.name());
            candidates = ChordDiagramGenerator
                    .generate(selection.chord(), tuning, ChordDiagramGenerator.DEFAULT_MAX_SPAN, selection.complexity())
                    .stream()
                    .filter(barrePreference::accepts)
