...
| 5 · README rewritten for users with a feedback section, English screenshots, developer notes moved to `docs/DEVELOPMENT.md` | `docs/readme-rewrite-in-english` | #234 | merged |

The stage is closed. Comments went from 6775 lines to about 1100, all external contracts in English. Code, tests, messages, docs and commits are in English. The interface ships in Spanish and English (v0.60.0), verified by launching the built jar under en_US and es_AR. Left for later: English wording that deserves a pass against the manual ("Triplet feel" vs "Triplet Feel", "Measure Properties" vs "Bar Properties").
